...
export class ProfileComponent implements OnInit {
  ...
  loadProfile() {
    ...
    this.http.get<UserProfile>(`http://quizrush-9.onrender.com/api/profile/${encodeURIComponent(username)}`)
      .subscribe({
        next: (data) => {
          this.profile = data;
          this.loading = false;
  ...
  saveUsername() {
    ...
    this.auth.updateDisplayName(this.newUsername.trim())
      .then(() => this.http.put('http://quizrush-9.onrender.com/api/profile/username', {
        currentUsername,
